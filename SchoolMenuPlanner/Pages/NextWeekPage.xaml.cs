...
namespace SchoolMenuPlanner
{
    public partial class NextWeekPage : Page
    {
        ...
        private void LoadSavedMenu()
        {
            try
            {
                // Загружаем меню для следующей недели
                var savedMenu = _context.DailyMenus
                    .Include(dm => dm.Dish)
                    .Where(dm => dm.WeekNumber == _nextWeekNumber && dm.Year == _yearForNextWeek)
                    .ToList();
        ...
        private async void SaveMenu_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                SaveDayMenu(1, "Monday");
                SaveDayMenu(2, "Tuesday");
                SaveDayMenu(3, "Wednesday");
                SaveDayMenu(4, "Thursday");
                SaveDayMenu(5, "Friday");

                await _context.SaveChangesAsync();
                MessageBox.Show("Меню на следующую неделю успешно сохранено!", "Успех", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
        ...
        private void SaveMealSelection(int dayOfWeek, string mealType, string comboBoxName, string categoryType)
        {
            ...
            if (comboBox?.SelectedItem is Dish selectedDish)
            {
                var dailyMenu = new DailyMenu
                {
                    ...
                    WeekNumber = _nextWeekNumber,
                    Year = _yearForNextWeek
                };
